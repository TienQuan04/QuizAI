...
//  CHECK ADMIN UPDATE
            // CHECK ADMIN UPDATE
            boolean updated = db.isQuestionUpdated(question);

            boolean deleted = db.isQuestionDeleted(question);
...
// ===== UPDATED =====
            if(updated){
                ...
                else if(latestCorrect == 1) label = "B";
                else if(latestCorrect == 2) label = "C";
                else if(latestCorrect == 3) label = "D";

                tvUpdated.setText(
                        "⚠ Admin đã cập nhật lại câu này.\nĐÁP ÁN MỚI CỦA HỆ THỐNG LÀ: " + label
                );

                tvUpdated.setTextColor(
                        Color.parseColor("#F59E0B")
                );

                tvUpdated.setTextSize(14);

                tvUpdated.setPadding(20, 0, 0, 20);

                container.addView(tvUpdated);
            }

// ===== DELETED =====
            if(deleted){

                TextView tvDeleted = new TextView(this);

                tvDeleted.setText(
                        "❌ Câu hỏi này đã bị admin xoá khỏi hệ thống"
                );

                tvDeleted.setTextColor(Color.RED);

                tvDeleted.setTextSize(14);

                tvDeleted.setPadding(20, 0, 0, 20);

                container.addView(tvDeleted);
            }
